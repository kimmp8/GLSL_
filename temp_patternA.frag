...
vec2 offset(vec2 coord){ // 실제 coord 좌표계는 왼쪽 아래 0,0으로 두고 0.5씩 x,y에 더해서 가운데 좌표계로 바꾸는 함수이다.
    vec2 uv;
    uv.x = coord.x > 0.5 ? coord.x - 0.5 : coord.x + 0.5;
    ...
    */
    ...
    uv.y = coord.y > 0.5 ? coord.y - 0.5 : coord.y + 0.5;
...
void main(){
    ...
    coord *= rotate(PI*0.25);
    float boundry = 0.01;
    float size = 0.05;
    vec3 color = vec3(box(offsetCoord, vec2(0.5), vec2(0.5), 0.02) - box(coord, vec2(size + boundry), vec2(0.0), 0.012) + 2.0 * box(coord, vec2(size), vec2(0.0), 0.012));
